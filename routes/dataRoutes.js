...
router.post('/payments/confirm', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (subscription) {
      ...
      // If PaymentIntent succeeded, attach payment method to customer
      if (paymentIntent?.status === 'succeeded' && subscription?.customer) {
        try {
          ...
          if (attachErr.code === 'resource_already_exists') {
            ...
            console.warn('⚠️ Could not attach payment method to customer (non-critical):', attachErr.message);
          }
        }
      }
...
async function updateSubscriptionInDatabase(userId, subscriptionStatus, plan, paymentIntentId, paymentMethod, subscriptionId, customerId, currentPeriodStart, currentPeriodEnd) {
  ...
  if (subscriptionId) {
    ...
    if (paymentIntent && (paymentIntent.status === 'succeeded' || paymentIntent.status === 'processing')) {
      ...
      // Attach payment method to customer after PaymentIntent succeeds
      if (paymentIntent.status === 'succeeded' && customerId) {
        try {
          const paymentMethodId = paymentIntent.payment_method;
      ...
      try {
        if (stripe && process.env.STRIPE_SECRET_KEY) {
          ...
          if (stripeSubscription.current_period_end) {
            ...
            // Format next invoice from Stripe subscription (for immediate display)
            ...
            if (formattedNextInvoice) {
              ...
              // Optionally save to database for caching (but don't fail if it doesn't work)
              try {
                ...
                const updateFields = ['current_period_end = @periodEnd', 'next_invoice = @nextInvoice', 'updated_at = SYSDATETIMEOFFSET()'];
                ...
                console.warn('⚠️ Could not save next_invoice to database (non-critical):', dbErr.message);
                // Continue - nextInvoice is still available for this response
              }
            } else {
              // Update database with dates only (next_invoice will be calculated on next status check)
              const updateRequest = pool.request();
              updateRequest.input('userId', mssql.Int, parseInt(userId, 10));
              updateRequest.input('periodEnd', mssql.DateTimeOffset, nextBillingDate);
